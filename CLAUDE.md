...
13. **camera / drawing / render** — `visibleRange()` computes the on-screen tile rect (capped at 150×150, 120 in cool mode); ground, procedural forest trees, and entities are all culled to it. The static ground (tiles, scorch, lane markings) renders into an offscreen canvas (`groundCache`) and is blitted per frame; it invalidates via `markGround()` (called by `setTile`/`clearTile`/scorch/import) plus camera/lighting key changes — animated ground (water ripples, bridges, congestion tints) draws live in `drawGroundLive`. Elevated objects go through the pooled `itemPool` depth-sorted by `x+y`: new drawable things need an `IT_*` kind, a `pushItem` call in `render()`, and a case in `drawItem()`. Paused + no input/animation frames skip repainting entirely (`paintDirty`).
...
`tests/run.js` extracts the `<script>` from `play/index.html`, stubs `window/document/canvas` with proxies, appends a `global.__h={...}` hook export, and drives `requestAnimationFrame` manually (33ms steps; queued, so one-shot rAF users can't clobber the frame loop). To expose new internals to tests, add them to the hook list in `tests/run.js`, not to the game file.
